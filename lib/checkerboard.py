...
class CheckerBoard(Board):
    ...
    def is_terminal(self):
        ...
        if not piececounts[0]:
            ...
            winner = None
            # Check for draws
            # Draws occur when neither player has advanced a pawn in more than drawthreshN
            # moves and there has not been a capture within the last drawtrhreshN moves.
            terminal = \
    ...
    def get_action_str(cls, action):
        ...
        for posn in action[1:]:
            if len(posn) == 2:
                ...
                strings.append("to %s capturing %s"%(posn[0:2], posn[2]))
        return " ".join(strings)
